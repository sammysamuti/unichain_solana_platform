...
export class FoundItemController {
  ...
  async createFoundItem(req: Request, res: Response) {
    try {
      const foundItem = await foundItemService.createFoundItem({
  ...
  async getFoundItems(req: Request, res: Response) {
    try {
      ...
      const foundItems = await foundItemService.getFoundItems(filters);
  ...
  async getFoundItemById(req: Request, res: Response) {
    try {
      const foundItem = await foundItemService.getFoundItemById(Number(req.params.id));
      return res.status(200).json({
  ...
  async getMyFoundItems(req: Request, res: Response) {
    try {
      const foundItems = await foundItemService.getFoundItemsByFinderId(Number(req.params.finderId));
      return res.status(200).json({
        message: foundItems.length ? 'User found items retrieved successfully' : 'No found items found for this user',
        error: null,
        data: foundItems
  ...
  async deleteFoundItem(req: Request, res: Response) {
    try {
      await foundItemService.deleteFoundItem(Number(req.params.id), Number(req.body.finderId));
      return res.status(200).json({
